...
namespace Comp4_Project
{
    ...
    {        
        private Atom[] atoms = new Atom[84]; //initialise the array of atoms
        private List<Neutron> neutronList = new List<Neutron>(); //initialise the list of neutrons

        private const int OFFSET = 170;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < startNeutrons; i++)
            {
                ...
                neutron.SetVelocityX(3);//select a random velocity in the x direction
                neutron.SetVelocityY(2);//same in the y

                neutron.SetXPos(random.Next(0, ClientSize.Width - (Neutron.Size + OFFSET)));
                neutron.SetXPos(random.Next(0, ClientSize.Height - Neutron.Size));
            ...
            int xDiff = (ClientSize.Width - (Atom.Size + OFFSET)) / numberOfAtomsPerRow; //to calculate the difference in the width of each x of the atoms
            int yDiff = (ClientSize.Height - Atom.Size) / numberOfAtomsPerColumn; //to calculate the difference in the height of each x of the atom
        ...
        private void MoveBalls()//procedure to move all neutrons
        {
            ...
            {
                neutron.Move(ClientSize.Width - OFFSET, ClientSize.Height); //Move all neutrons first
            }
                ...
                foreach (Neutron neutron in neutronList)
                {
                    if (atom.InteractsWith(neutron))//only trigger this block if the neutron interacts with an atom
                    {
                        ...
                        for (int i = 0; i < 2; i++)
                        {
                            ...
                            RotateNeutron(newNeutron, neutron, radians);
        ...
        private void drawBalls(PaintEventArgs e)//draws all screen objects
        {
            ...
            {
                e.Graphics.FillEllipse(ball.PickBrush(), ball.GetXPos(), ball.GetYPos(), Neutron.Size, Neutron.Size);
                e.Graphics.DrawEllipse(Pens.Black, ball.GetXPos(), ball.GetYPos(), Neutron.Size, Neutron.Size);
            }

            foreach (Atom a in atoms)//draw all atoms as circles with the same diamiter as the width of the atom
            {
                e.Graphics.FillEllipse(a.PickBrush(), a.GetXPos(), a.GetYPos(), Atom.Size, Atom.Size);
                e.Graphics.DrawEllipse(Pens.Black, a.GetXPos(), a.GetYPos(), Atom.Size, Atom.Size);
            }
        }
    }
}
